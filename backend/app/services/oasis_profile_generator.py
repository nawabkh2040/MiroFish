...
Converts entities from Zep knowledge graph into Agent Profile format required by OASIS simulation platform

Optimization improvements:
1. Call Zep retrieval functionality to enrich node information
...
class OasisAgentProfile:
    ...
    def to_reddit_format(self) -> Dict[str, Any]:
        ...
        profile = {
            ...
            "username": self.user_name,  # OASIS lib requires field name as username (no underscore)
            "name": self.name,
            "bio": self.bio,
...
class OasisProfileGenerator:
    ...
    Converts entities from Zep knowledge graph into Agent Profiles required for OASIS simulation
    
    Optimization features:
    1. Call Zep graph retrieval functionality to get richer context
    ...
    # Group/institutional entity types (need to generate group representative personas)
    GROUP_ENTITY_TYPES = [
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Zep graph hybrid search to get rich information about entity
        
        Zep doesn't have built-in hybrid search interface, need to search edges and nodes separately then merge results.
        Use parallel requests for simultaneous search to improve efficiency.
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        """
        Build comprehensive context information for entity
        ...
        if zep_results.get("facts"):
            # Remove duplicates: exclude already existing facts
    ...
    def _generate_profile_with_llm(
        ...
        # Retry LLM generation up to max attempts
        max_attempts = 3
        last_error = None
            ...
            try:
                ...
                try:
                    ...
                    if "persona" not in result or not result["persona"]:
                        result["persona"] = entity_summary or f"A {entity_type} named {entity_name}."
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # Simple check: if last character is not a comma or closing bracket, string may be truncated
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        if json_match:
            json_str = json_match.group()
             ...
             # 3. Handle line break issues in strings
            # Find all string values and replace line breaks within them
            def fix_string_newlines(match):
                s = match.group(0)
                # Replace actual line breaks in strings with spaces
                s = s.replace('\n', ' ').replace('\r', ' ')
...
- 机构账号发言要符合其身份定位"""
    ...
    def generate_profiles_from_entities(
        ...
        # Realtime file save helper function
        def save_profiles_realtime():
            """Save generated profiles to file in realtime"""
            import os  # Ensure os module is available in this nested function
            ...
            with lock:
                ...
                try:
                    if output_platform == "reddit":
                        # Reddit JSON format - use atomic write to prevent incomplete reads
                        profiles_data = [p.to_reddit_format() for p in existing_profiles]
                        temp_path = realtime_output_path + ".tmp"
                        with open(temp_path, 'w', encoding='utf-8') as f:
                            json.dump(profiles_data, f, ensure_ascii=False, indent=2)
                        os.replace(temp_path, realtime_output_path)
                    else:
                        # Twitter CSV format - use atomic write
                        import csv
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                # Print generated persona to console and log in realtime
        ...
        logger.info(f"Starting to generate {total} agent personas in parallel (parallel count: {parallel_count})...")
        print(f"\n{'='*60}")
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        """Print generated persona to console in realtime (full content, no truncation)"""
        separator = "-" * 70
        ...
        # Only output to console (avoid redundancy, logger will no longer output full content)
        print(output)
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        - user_id: User ID (starting from 0 based on CSV row order)
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for idx, profile in enumerate(profiles):
                ...
                # Handle line breaks (replace with spaces in CSV)
                ...
                row = [
                    ...
                    user_char,              # user_char: Full persona (internal LLM usage)
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Uses format consistent with to_reddit_format(), ensures OASIS can read correctly.
        ...
        # Use atomic write: write to temp file, then rename (prevents incomplete reads)
