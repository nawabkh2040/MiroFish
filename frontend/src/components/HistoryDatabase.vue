        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 "Run Simulation" and Step 5 "Deep Interaction" must be started whilst running and do not support historical playback</span>
            </div>
          </div>
        </div>
      </Transition>
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate number of cards in current row, ensuring each row is centred
    ...
    // Collapsed state: fan-stack layout
    const transition = 'transform 700ms cubic-bezier(0.23, 1, 0.32, 1), opacity 700ms cubic-bezier(0.23, 1, 0.32, 1), box-shadow 0.3s ease, border-color 0.3s ease'
    ...
    const x = offset * 35
    // Adjust start position, close to title but maintaining appropriate spacing
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // If animating, only record the state and handle after animation ends
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check whether pending state still needs executing (may have been overridden by subsequent scroll)
          ...
          // Unlock after animation and check for any pending state changes
          setTimeout(() => {
            isAnimating = false
            
            // After animation ends, check for new pending state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay briefly before executing to avoid switching too rapidly
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin so the viewport bottom shrinks upwards, requiring more scrolling to trigger expansion
...
// Watch for route changes, reload data when returning to the home page
...
onMounted(async () => {
  // Ensure DOM has fully rendered before loading data
  await nextTick()
  await loadHistory()
...
.grid-pattern {
  ...
  /* Position from top-left; height changes only expand downwards, not affecting existing grid position */
...
/* Progress status colours - footer */
