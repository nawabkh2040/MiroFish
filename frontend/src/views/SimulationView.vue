 ...
 * When users return from Step 3 to Step 2, we assume they want to exit.
 */
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      addLog('Detected running simulation environment, attempting graceful shutdown...')
      ...
      try {
        ...
        if (closeRes.success) {
          addLog('✓ Simulation environment shut down')
        } else {
          addLog(`Failed to shut down simulation environment: ${closeRes.error || 'Unknown error'}`)
          // If graceful shutdown fails, try to force stop
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(`Exception while shutting down environment: ${closeErr.message}`)
    ...
    // Failing to check environment status should not block the flow
    console.warn('Failed to check simulation status:', err)
  }
}

/**
 * Force stop the simulation process.
 */
const forceStopSimulation = async () => {
  try {
    ...
    addLog(`Exception while force-stopping simulation: ${err.message}`)
  }
}
